...
export async function PUT(request, { params }) {
  try {
    ...
    const result = await db.collection("topproducts").findOneAndUpdate(
      { _id: new ObjectId(id) },
      { $set: updateData },
    ...
    console.error("Update toprated product error:", error);
...
export async function DELETE(request, { params }) {
  try {
    ...
    const result = await db.collection("topproducts").deleteOne({ _id: new ObjectId(id) });
    ...
    console.error("Delete toprated product error:", error);
    return Response.json({ message: "Internal server error" }, { status: 500 });
  }
}
